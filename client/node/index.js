...
app.get('/:uid', function (req, res) { 
      unirest.get('http://colab-sbx-88.oit.duke.edu:3000/patient/' + req.params.uid)
      .end(function (response) {
            unirest.get('http://0.0.0.0:3000/predict')
            .query({preds: transform(response.body)})
            .end(function (response) {
                  res.send(response);
      });
  });
...
app.get('/predict/:uid', function (req, res) {
      unirest.get('http://colab-sbx-88.oit.duke.edu:3000/patient/' + req.params.uid)
      .end(function (response) {
            var options = {
                  mode: 'json',
                  args: [ transform(response.body) ]
            };
